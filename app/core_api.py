"""Core: веб-админка (FastAPI + Jinja2) + внутренний API регистрации нод."""
...
from .core_service import (check_node, create_cred, delete_cred, delete_node, get_cred,
                           get_node, get_node_by_name, list_creds, list_nodes, sync_node,
                           upsert_node)
...
async def lifespan(app: FastAPI):
    ...
    if config.ROLE in ("core", "all") and config.BOT_TOKEN:
    ...
    log.info("core запущен: role=%s, port=%s", config.ROLE, config.PORT)
    yield
    for t in tasks:
        t.cancel()
...
app = FastAPI(title="proxy_rail core", version=config.APP_VERSION,
